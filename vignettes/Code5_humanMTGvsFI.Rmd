...
This script identifies common and distinct CF-associated genes in human middle temporal gyrus as compared with human fronto-insula, and compares the proportions of cells across clusters for each brain region.  
...
### Find common and distict CF-associated genes in FI and MTG
  
We have already shown in the previous code that a handful of genes are enriched in CF cells relative to other subclasses in multiple regions in mouse and human.  Now we want to extend this result to see if we can find an expanded list when we only consider human, and also to identify genes which may be specific to FI (and therefore potentially associated with diversified morphologies).
...
Next, find the top marker genes for the CF cluster in each data set (Exc FEZF2 GABRQ in FI and Exc L4-5 FEZF2 SCN4B in MTG).  Note that we are using slightly different statistical criteria and comparison types as we did in the cross-species analysis, and therefore the resulting gene lists do not perfectly align.
...
Here we calculate the fraction of excitatory cells from layer 5 that map to the CF clusters in MTG and FI.
...
Note that there is a much higher percentage of CF-associated excitatory cells in FI than in MTG.  These values are also consistent with results from ISH.  
